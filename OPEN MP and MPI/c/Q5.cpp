...
__global__ void myAdder(int * arr, int *sum,int N)
...
int main()
{
	...
	*sum_h=0;
	...
	cudaMemcpy(array_d,array_h,cudaMemcpyHostToDevice);
	cudaMemcpy(sum_d,sum_h,cudaMemcpyHostToDevice);
	...
	cudaMemcpy(sum_h,sum_d,cudaMemcpyDeviceToHost);
